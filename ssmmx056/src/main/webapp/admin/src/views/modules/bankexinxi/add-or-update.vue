<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="班课编号" prop="bankebianhao">
            <el-input v-model="ruleForm.bankebianhao" 
                placeholder="班课编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.bankebianhao" label="班课编号" prop="bankebianhao">
              <el-input v-model="ruleForm.bankebianhao" 
                placeholder="班课编号" readonly></el-input>
          </el-form-item>
        </div>
...
export default {
  ...
  methods: {
    ...
    banjiChange () {
      this.$http({
        url: `follow/xuesheng/banji?columnValue=`+ this.ruleForm.banji,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `bankexinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.bankexinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
